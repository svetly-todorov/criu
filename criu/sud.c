...
int dump_sud_per_core(pid_t tid_real, ThreadCoreEntry *tc)
{
	...
	if (entry->mode == SYS_DISPATCH_ON) {
		thread_core->has_sud_mode = true;
		thread_core->sud_mode = entry->mode;
		thread_core->has_sud_setting = true;
		thread_core->sud_setting = entry->img_setting_pos;
	}
...
int dump_sud(void)
{
	...
	struct sud_entry *entry;
	size_t img_setting_pos = 0, nr_settings = 0;
	struct rb_node *node;
	int ret;

	/* Get the number of entries in the collect ring buffer */
	for (node = rb_first(&sud_tid_rb_root); node; node = rb_next(node)) {
		entry = rb_entry(node, struct sud_entry, node);
		nr_settings += 1;
	}

	/* Allocate space for dumping the thread settings */
	se.n_sud_settings = nr_settings;
	if (nr_settings) {
		se.sud_settings = xmalloc(sizeof(*se.sud_settings) * nr_settings);
		if (!se.sud_settings)
			return -1;
	}
	...
	for (node = rb_first(&sud_tid_rb_root); node; node = rb_next(node)) {
		...
		if (entry->mode == SYS_DISPATCH_ON) {
			se.sud_settings[img_setting_pos].selector = entry->selector;
			se.sud_settings[img_setting_pos].offset = entry->offset;
			se.sud_settings[img_setting_pos].len = entry->len;
...
// 		if (thread_core->sud_setting >= sud_img_entry->n_sud_settings) {
// 			pr_err("Corrupted sud setting index on tid %d (%u > %zu)\n", item->threads[i].ns[0].virt,
// 			       thread_core->sud_setting, sud_img_entry->n_sud_settings);
// 			return -1;
// 		}
 ...
 * image streamer.
 */
int restore_sud_per_core(pid_t tid_real, ThreadCoreEntry *thread_core)
{
	SudSetting *ss;
